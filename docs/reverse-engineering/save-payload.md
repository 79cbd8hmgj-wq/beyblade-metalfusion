...
## Task 8 migration boundary

Original saves without a valid custom extension are parsed byte-preservingly and receive in-memory sanitized defaults in the tooling model. The editor writes a new output file only and preserves bytes outside the tail extension.
